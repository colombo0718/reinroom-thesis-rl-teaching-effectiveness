...
實驗以 RR 組（n=18）與 Colab 組（n=12）兩班學生為對象，於兩天連續課程（Day 1 概念導入、Day 2 進階任務）中執行同一套教學節奏與評量工具。評量涵蓋四個層次：（1）概念理解前後測；（2）任務完成成效（完成率與完成時間雙指標）；（3）學習參與度與平台使用回饋（SUS、NASA-TLX、自陳量表、開放題）；（4）課堂行為觀察與平台改進回饋。
...
本研究貢獻有三：（1）以實證資料證明「視覺化互動式 RL 教學平台」可在不犧牲概念理解的前提下，於遷移能力與操作直覺感上產生獨特優勢；（2）提出「完成率 + 完成時間」雙指標評量法，作為平台類教學工具捕捉「無指引情境下遷移能力」的具體做法；（3）整理出五項對未來「視覺化互動式教學平台」設計之具體建議，提供後續研究者與開發者參考。
